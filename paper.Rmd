...
of package sources. Further, tools like [RStudio Package Manager](todo) add
...
private usage information captured by [Package Manager](todo).
...
As was discussed in a [blog post on this topic](todo), the researcher's
...
[`checkpoint`](todo) package or by specifying a Package Manager transaction:

```r
options("repos" = "https://mypackagemanager.com/cran/773")
```

Further, the researcher might decide to opt to use [`packrat`](todo) or a
...
welcome at [community.rstudio.com](todo) for how to manage projects.
...
The way that is done in the digital realm is with virtual machine images
([AMIs](todo), [VMIs](todo), [GMIs](todo)) or a containerization tool like
[Docker](todo) or [Singularity](todo). In this paradigm, the researcher desires
to "snapshot" the entire kitchen (R packages, system dependencies, operating
system) and all ingredients, utensils, or other components of the process.
...
packages as discussed above. When moving into a ephemeral paradigm like Docker,
